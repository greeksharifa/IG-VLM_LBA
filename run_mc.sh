...
CUDA_VISIBLE_DEVICES=${device} python eval_llava_multiplechoice.py --path_qa_pair_csv ./data/multiple_choice_qa/${dataset_name}_sub_qas_val_fewshot_vqaintrospect_unique.csv --path_video ./data/${dataset_name}/videos/%s.mp4 --path_result ./result_${dataset_name}/${exp} --llm_size 7b  --dataset_name ${dataset_name} --num_data ${num_data} --sub_qa_index ${exp}
